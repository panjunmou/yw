...
public class Office2Swf {
    ...
    public static String office2Swf(String inputFilePath, String outputPath) throws Exception {
        ...
        if (null == outputSwfPath || "".equals(outputSwfPath.trim())) {
            outputPdfPath = inputFilePath.replace("." + postfix, ".pdf");
        } else {
            String outputFix = getPostfix(outputSwfPath);
            outputPdfPath = outputSwfPath.replace("." + outputFix, ".pdf");
        }

        boolean isSucc;
        if (!postfix.equals("pdf")) {
            isSucc = Office2PDF.openOffice2Pdf(inputFilePath, outputPdfPath);
        } else {
